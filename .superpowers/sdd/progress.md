...
P3 QA-CHECKPOINT C: complete (QA doc 0c4b1985; 6 scenarios all real-executed incl. sandboxed-HOME bootstrap run with double-run idempotency; F1 dialyzer :eex + F2 priv-glob-ships-PLT-blobs BOTH FIXED by controller [scoped priv/runtime_bootstrap, tarball 72KB/0 blobs verified]; injection matrix + merge matrix + bootstrap-on-handle all PASS).
