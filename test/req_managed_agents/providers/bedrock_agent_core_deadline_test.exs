defmodule ReqManagedAgents.Providers.BedrockAgentCoreDeadlineTest do
  ...
  describe "legacy poll opts" do
    # The N+1 derivation itself is pinned in WaitBudgetTest, not here: at any
    # cadence cheap enough for a provider-level test the derived deadline sits on
    # its floor, so the count binds and the derivation decides nothing.
