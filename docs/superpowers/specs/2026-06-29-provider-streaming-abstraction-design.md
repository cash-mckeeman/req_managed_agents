...
- **Managed Agents:** `normalize/1` populates `text` by concatenating the `text` blocks of every `agent.message` event in the turn. The shape — `%{"type" => "agent.message", "content" => [%{"type" => "text", "text" => …}]}` — was **verified** against Anthropic's Managed Agents docs and the `biai-platform` consumer (not guessed); non-text blocks (thinking / tool_use) are skipped. Note: the drivers' result shape `{terminal, stop_reason, events}` does not yet surface `text` — propagating it to driver results is a separate, optional enhancement.
